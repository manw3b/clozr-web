...
**⏯️ RETOMAR ACÁ (última sesión: 2026-06-14):** la webapp está LIVE en https://www.clozr.online (landing + CRM funcionando). Se acaba de terminar el CÓDIGO de "Login con Google" (front pusheado y auto-deployado; worker pusheado pero NO deployado). **El usuario quedó por hacer 3 pasos manuales para activarlo:** (1) crear OAuth client en Google Cloud Console (redirect URI `https://clozr-auth.pyter-import.workers.dev/auth/google/callback`, publicar consent screen); (2) `cd /home/pyter/clozr/cf-worker && npx wrangler secret put GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET`; (3) `npx wrangler deploy`. Cuando avise, verificar el flujo end-to-end. Próximos temas en cola: abrir el login por email a todos (verificar `clozr.online` en Resend) y, más adelante, billing (Mercado Pago/Lemon Squeezy). Detalle completo abajo.
