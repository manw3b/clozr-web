...
- **Unirse con código + alias + email de invitación (2026-06-23)** — el dueño genera un **código de tienda** (Equipo → "Código de la tienda") y cualquiera logueado se une como empleado desde el onboarding ("Ya me invitaron — entrar con código"). Backend: `POST /workspaces/:id/join-codes` + `POST /join` (seat-gate, expiración, rol acotado a no-owner, tabla lazy `workspace_join_codes`). El chip de cuenta muestra **alias** (`displayName`) y oculta el email (queda en tooltip); en Ajustes "Tu nombre" → "Tu alias". Email de invitación rehecho: banner Clozr, **sin** el correo del invitador, flujo por la web, sin link a GitHub.
...
1. **Permisos reales** — ✅ **completo (LIVE, 2026-06-23)**: frontend (gating en vistas/sidebar/atajos + modo lectura en modales) **y enforcement server-side en el Worker** — `cf-worker/src/permissions.ts` (matriz espejo del front) + `requirePerm(role, perm)` en TODOS los handlers de escritura (customers/sales/pipeline/cash/inventario/tasks/settings/team, incl. `catalog-stock` ya alineado a la matriz web). Un `viewer` recibe 403 en cualquier POST/PATCH/DELETE; un `vendedor`, 403 en settings/team/inventory/reports.
2. **Vendedor "ve solo lo suyo"** (alcance de datos) — ✅ **completo (LIVE, 2026-06-23)**: columna `owner_id` en `customers`/`sales`/`pipeline_items` (+ backfill al owner e índices), seteada al creador en cada INSERT; los GET filtran por `owner_id` si el rol es `vendedor` (managers/viewer ven todo) y los PATCH/DELETE chequean propiedad (`assertOwns*` → 403 si no es suyo). Stages/catálogo/tareas/caja quedan compartidos a propósito; Mi Día/Deudas heredan el scope.

> **Nota de acceso (actualizada 2026-06-23):** el Worker (`manw3b/clozr` → `cf-worker/`) **ya está autorizado** en el entorno y se deploya con `wrangler deploy`. Tareas 1, 2 y 3 del spec **ya aplicadas y LIVE**; `BACKEND-equipos-spec.md` queda como referencia histórica.
